...
jq -e '.required_reviewers >= 1 and .prevent_self_review == true and .deployment_branch_policy.allowed_branch_pattern == "main" and .deployment_branch_policy.allowed_tag_pattern == "v*" and (.required_secrets | sort) == (["RELEASE_TAG_PUBLIC_KEY_BASE64"] | sort)' .github/environments/release-policy.json >/dev/null
...
grep -Fq 'gitsign.rekorMode offline' "$release_workflow"
