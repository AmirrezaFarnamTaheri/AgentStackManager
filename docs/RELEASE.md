...
Before checkout, a tag-push release validates through the GitHub API that the ref is an annotated tag object with a verified signature. Manual dispatch checks out current `main`. A `workflow_run` release checks out the successful Verify run's `head_sha` and proceeds only when that commit still equals fetched `origin/main`; it fails safely if `main` has advanced. Automatic entry points infer the next semantic version from Conventional Commit signals and create a keyless Sigstore-signed annotated tag. The workflow recovers an interrupted workflow-owned unpublished tag and skips an already published commit. It then builds and signs both Windows architectures, creates GitHub provenance and SBOM attestations, executes the signed ARM64 artifacts on a native ARM64 runner, verifies all downloaded checksums and attestations, and only then publishes the GitHub Release with `gh release create`.
